...
const Experience = () => {
  return (
    <div className='experience'>
      <div className="expHeaderTitle"><span>Experience</span></div>
      {
        experienceData.map(expItem => (<React.Fragment key={expItem.companyName}>
          <div className='expSection'>
            ...
            <div className='companyAndRole'>
              ...
              <div className="skillsUsed">
                {expItem?.skillsUsed && expItem?.skillsUsed?.length > 0 && expItem?.skillsUsed.map(skill => <span key={skill} className='expSkill'>{skill}</span>)}
              </div>
            </div>
          </div>
        </React.Fragment>
